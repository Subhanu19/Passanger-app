...
export default function ScheduleScreen() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = webSocketService.subscribe((data) => {
      ...
      let progress = distFromCurrent / totalDist;
      progress = Math.max(0, Math.min(1, progress));

      // Update animated bus position
      ...
      if (progress >= 1 && currentStopIndex < stops.length - 1) {
        ...
        if (flatListRef.current) {
          flatListRef.current.scrollToOffset({
            offset: (currentStopIndex + 1) * ITEM_HEIGHT - ITEM_HEIGHT,
            animated: true,
          });
        }
      }
    ...
    return () => unsubscribe();
  }, [busData.stops, currentStopIndex, translateY]);
  ...
  const renderStop = ({ item, index }) => {
    ...
    return (
      <View style={styles.stopContainer}>
        <View style={styles.timeline}>
          ...
          <Animated.View
            style={[
              styles.progressLine,
              {
                height: lineProgress[index].interpolate({
                  inputRange: [0, 1],
        ...
        <View
          style={[styles.stopDetails, { borderBottomColor: theme.border }]}
        >
    ...
    <View style={styles.container}>
      <View style={styles.headerSection}>
        ...
        <Text style={styles.subtitle}>
          Bus #{busData.bus_id} - Live Tracking
        </Text>
      </View>
      ...
      <View style={styles.scheduleContainer}>
        ...
        {/* 🚍 Bus inside timeline (scrolls naturally with list) */}
      ...
      <View style={[styles.footer, { borderTopColor: theme.border }]}>
        ...
        <Text style={[styles.connectionStatus, { color: theme.textLight }]}>
          Next stop:{" "}
          {busData.stops?.[currentStopIndex]?.location_name || "N/A"}
        </Text>
      </View>
    </View>
  );
...
const createStyles = (theme) =>
  StyleSheet.create({
    ...
    headerSection: {
      padding: 50, 
       zIndex: 100, // this hides the bus icon when i pass over it  
      paddingBottom: 16,
      borderBottomWidth: 1,
    ...
    // 🚍 Bus scrolls with list and moves only by WebSocket updates
    busLayer: {
      position: "absolute",
      top: 0,
      left: 20,
